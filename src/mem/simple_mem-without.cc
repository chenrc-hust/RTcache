    ...
    if (needsResponse) {
       ...
       //cerr<<pkt_read<<" pkt->read"<<endl;
        ...
        if(what_twl==0){//无磨损均衡
            ...
            //更新统计表
            stats.pagelife[min_it->first]+=16;stats.pagelife[max_it->first]+=16;
            /*  pageWriteCounts是twl=3时监控寿命的数组*/
            pageWriteCounts[min_it->first]+=16;pageWriteCounts[max_it->first]+=16;//页面实际写入次数
            if(bool_cache){
            ...
            //cerr<<"swap1"<<endl;
                ...
                if(bool_cache){
                    ...
                    temp_1 = mm.queryRemap(returnValue[1]);
                    temp_2 = mm.queryRemap(returnValue[2]);
                }else{
                    swap(pageRemap[returnValue[1]],pageRemap[returnValue[2]]);
                    temp_1 = pageRemap[returnValue[1]];
                    temp_2 = pageRemap[returnValue[2]];
                }
                //地址没有问题，
            //cerr<<"error -1"<<endl;
                //统计页面的写入次数
            ...
            //cerr<<"error 1"<<endl;       
                ...
                if(bool_cache){
                    ...
                    temp_1 = mm.queryRemap(returnValue[1]);
                    temp_2 = mm.queryRemap(returnValue[2]);
                    temp_3 = mm.queryRemap(returnValue[3]);    
                    //统计页面1的写入次数
                    /* 测试两个映射表是否相同 */
                    ...
                    temp_1 = pageRemap[returnValue[1]];
                    temp_2 = pageRemap[returnValue[2]];
                    temp_3 = pageRemap[returnValue[3]];
                }
