...
//double  climberobj::gaussrand(double mu, double s1igma)
//{
...
//    return z0 * sigma + mu;
//};
climberobj::climberobj(uint64_t areasize,int threshold){
  	 this->maxpagenums = areasize;
  	 climbethreshold = threshold;
     cerr<<"climber swap"<<climbethreshold<<endl;
    ...
    /* 初始化基本参数：设置最大页面数 maxpagenums 为传入的 areasize，并初始化其他与算法执行相关的基础参数，
    如爬行阈值 climbethreshold、位移 climbershift、是否启用爬行 climberenable 等。 */
     //np.random.seed(0)
...
//     sort(p,p+2*areanums,greater<double>());
     ...
     sprintf(lifepath,"/home/chenrc/gem5-23.0.1.0/src/mem/life_%luG.dat", pagesize);
     printf("lifepath:%s\n",lifepath);
     ...
     in.close();
     /* 初始化页面寿命列表：为每个页面分配一个 lifenode 结构，该结构记录页面的地址和寿命。
     初始化两个列表 lifelist 和 lifelist2，以及排序后的列表 sortedlist。 */
...
uint64_t* climberobj::climber(uint64_t addr_temp, uint64_t counterv){
        ...
        uint64_t addr = this->maplist[addr_temp];//逻辑地址到物理地址
        ...
        /* 计算阈值：根据页面的物理地址addr获取本地Climber阈值localclimbethreshold。如果页面的访问次数counterv达到了这个阈值的倍数，
        并且Climber算法被启用（this->climberenable >= 1），则开始执行页面交换逻辑。 */
        if ((counterv % (localclimbethreshold) == 0) and (this->climberenable >= 1)){
            ...
            bool tarlabool2 = 0;
            bool tarlabool3 = 0;
            /* 定义目标索引0 (targetindex0)：这是一个特殊的目标索引，设定为内存区域最大索引indexmax。
             */
            uint64_t targetindex0 = indexmax;
            targetbool0 = 1;
            /* 使用getrandom函数在有效范围内生成一个随机数，这个随机数将用于选择目标页面。 */
...
uint64_t* climberobj::access2(uint64_t addr_temp,bool iswrite){
        if (iswrite == 0){
        	ans[0] = 0;
